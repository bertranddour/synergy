  ...
  const activeQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{
        ...
        schedule: Array<{ day: number; date: string; modeSlug: string; modeName?: string; completed: boolean }>
      }>
    },
  })
          ...
          programsQuery.data?.programs.map((program) => (
            <div key={program.slug} className="shadow-neo-well rounded-[1.8rem] bg-[var(--surface)] p-6">
              ...
              <div className="mt-3 flex items-center gap-3 text-xs text-[var(--text-tertiary)]">
                ...
                <span>·</span>
                <span>{program.frameworkNames?.join(', ') ?? program.frameworksRequired.join(', ')}</span>
              </div>
              <button
                type="button"
